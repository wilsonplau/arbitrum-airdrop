...
const TokenBalancesTable: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg bg-gray-800 p-8">
      <div>
        <h1 className="text-2xl text-white">Token Holders</h1>
        <h2 className="text-lg text-gray-400">
          Which addresses currently hold $ARB tokens?
        </h2>
      </div>
      <input
        type="text"
          ...
          <div>
            <p className="text-sm text-gray-300">
              ...
              <span className="font-medium">
                {pageNumber * pageSize + Math.min(1, balances.length)}
              </span>{" "}
              to{" "}
              <span className="font-medium">
                {pageNumber * pageSize + balances.length}
              </span>
            </p>
          </div>
          <div>
